...
CREATE MATERIALIZED VIEW IF NOT EXISTS onda.mv_usage_sends_daily
TO onda.usage_sends_daily AS
SELECT
    tenant_id, app_id,
    toDate(sent_at) AS day,
    channel, status,
    count() AS sends
FROM onda.message_log
GROUP BY tenant_id, app_id, day, channel, status;

-- 활성 유저 일 집계 (이벤트 기준) — MAU는 조회 시 30일 uniqCombined
CREATE TABLE IF NOT EXISTS onda.usage_active_users_daily
(
    tenant_id UUID,
    app_id    UUID,
    day       Date,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS onda.mv_usage_active_users_daily
TO onda.usage_active_users_daily AS
SELECT
    tenant_id, app_id,
