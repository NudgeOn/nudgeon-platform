...
- 대사: outbox 발송 의도 == N, `message_log` sent 고유 message_id == 멱등 키 == N, sent 행 == N(크래시로 유실된 로그를 재전달이 복구한 행 포함, 이중 로그 0), failed 0, 싱크 실수신 ≥ N(유실 0). 싱크 초과분(공급자 중복)은 경고로 보고하고 `PROVIDER_DUPES_FAIL=1`이면 실패로 다룬다.
...
| C (수정 후) | 3,000 | 10 (0.5s 간격) | 3 | 3,001 | **1** | 3,000/3,000 | 9 | **62s** |

C는 아래 세 수정 뒤의 실측이다: 리퍼 창 5분→60초(+노드 실행 하트비트, 기동 즉시 회수, `SKIP LOCKED`),
워커 PG 세션 `idle_in_transaction_session_timeout=30s`(죽은 워커가 잡은 행 잠금을 PG가 푼다 — 이것 없이는
리퍼가 그 잠금에 막혀 한 번은 637초까지 늦었다), 재전달 재기록 전 CH 점조회(같은 message_id의 sent 행이
둘이 되지 않는다 — B의 3,001행이 C에서 3,000행). 공급자 중복 1건은 그대로 at-least-once 창이다.
...
- 죽은 워커가 클레임한 `journey_states`는 리퍼가 `claimReap`(60초, 15초 주기) 뒤 회수한다. kill이 스케줄러 배치 중에 떨어지면 그 배치는 최대 75초 늦게 나간다(C의 수렴 62초). 산 워커의 클레임은 노드 실행마다 `claimed_at` 하트비트를 받아 회수되지 않고, 회수돼도 `claim_token` 펜스가 이전 워커의 쓰기를 막는다.
