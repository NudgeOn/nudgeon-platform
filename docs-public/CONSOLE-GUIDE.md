...
2026-09-18 기준 NudgeOn 콘솔 안내입니다. 캡처는 현재 콘솔 코드를 실행하고 API 응답을 예시 데이터로 대체해 만들었습니다. 계정·고객·발송 수치는 설명용이며, 캡처 과정에서 실제 메시지를 보내지 않았습니다. 설치된 버전과 권한에 따라 보이는 메뉴가 다를 수 있습니다.
...
고객 식별자(`external_id`) 또는 이메일을 정확히 입력해 고객을 찾고, 프로필·디바이스·최근 이벤트를 봅니다. 데이터 화면의 **이벤트** 탭에는 회원가입·로그인·구입·상품 조회·장바구니 담기·결제 시작 6개의 기본 이벤트가 준비되어 있습니다. 이벤트를 선택하면 권장 속성과 복사 가능한 연동 예시를 확인할 수 있습니다. SDK나 서버에서 전송하기 전까지 실제 이벤트는 발생하지 않습니다. **속성 사전**과 **수집 오류** 탭은 기존 수집 데이터를 표시합니다.

저니의 진입·전환·조건 분기·이벤트 대기와 세그먼트의 이벤트 조건에서도 **기본 이벤트 선택**을 사용할 수 있습니다. 선택 후 이름을 직접 입력해 커스텀 이벤트로 바꿀 수 있습니다. [이벤트 이름과 연동 규약](API.md#기본-이벤트)을 참고하세요.

**속성** 탭에서는 `first_name`, `last_name`, `email`, `phone`, `dob`, `gender`, `home_city`, `country`, `language`, `timezone`, `created_at` 11개 기본 키와 권장 형식·연동 예시를 확인합니다. 그 아래 속성 사전은 실제 수집된 값을 기준으로 표시됩니다. 기본 목록만으로 프로필이 자동 수집되지는 않습니다.

개발 담당자와 연동한 테스트 고객의 프로필에서 `country = KR`, `timezone = Asia/Seoul` 등을 확인한 뒤 세그먼트·저니 조건에서 **기본 속성 선택**을 사용하세요. `membership_level` 같은 커스텀 키도 직접 입력할 수 있습니다. `dob`는 `YYYY-MM-DD` 문자열이며 반복 생일 저니를 자동 생성하지 않습니다. `created_at`은 서비스 가입 시각입니다. `null` 전송은 값 삭제이며 푸시 수신 동의는 프로필과 별도로 관리합니다. [속성 규약](API.md#기본-사용자-속성)과 [네 SDK 연동 예제](https://developer.nudgeon.io/#events-attributes)를 참고하세요.

아래 데이터 캡처는 기본 이벤트·속성 목록 추가 전 화면입니다.
...
운영 노출·참여·닫힘과 실패 기록은 테스트 이력과 분리됩니다. 서버 기능 플래그와 신규 DB migration, 앱의 인앱 SDK 모듈 연결이 필요합니다. iOS SPM과 Android Maven의 코어·인앱 SDK 0.2.4가 공개되어 있으며, RN/Flutter의 인앱 API는 후속 작업입니다. [작업실 안내](IN-APP-WORKBENCH.md)와 [캠페인 게시·SDK 연결](IN-APP-CAMPAIGNS.md)을 참조하세요.

### 앱 시작 전면 광고 — APP LAUNCH ADS (iOS·Android)

OS 런치 화면 다음에 불투명 전면 광고를 **기본 4초(3~5초 설정 가능)** 표시하고 자동으로 메인 화면으로 이동합니다. 시작 광고에는 네이티브 닫기·오늘 하루 안 보기 버튼이 없습니다. SDK 0.2.4와 앱의 시작 화면 연결이 먼저 필요합니다.

1. **인앱 이벤트 → 시작 전면 광고 예제**를 선택하거나 HTML/ZIP을 업로드하고 **저장·검증**합니다. 웹 소스와 자산은 NudgeOn에서 관리합니다.
2. 미리보기와 연결한 iOS·Android 기기에서 콘텐츠를 확인합니다. 작업실 테스트는 네이티브 닫기로 마칩니다. 이 단계는 콘텐츠 검수이며 시작 광고의 자동 종료 테스트가 아닙니다.
3. **캠페인 관리**에서 같은 소스 버전의 대상 OS별 **검수 통과**를 완료합니다. 표시 조건 **앱 실행 직후**, 기간·빈도·시간대를 설정하고 **초안 저장 → 저장된 버전 게시**합니다. 한국 날짜 기준 하루 제한은 `Asia/Seoul`을 선택합니다. 검수·게시는 Admin/Owner 권한이 필요합니다.
4. 통제된 테스트 앱의 프로세스를 종료하고 다시 실행하여 런치 → 전면 광고 → 약 4초 후 메인, 운영 표시·노출·`dismiss(auto_dismiss)`를 확인합니다. 단순 백그라운드 복귀는 새 시작 기회가 아니며 프로세스 재시작도 서버 빈도·숨김 제한을 초기화하지 않습니다.
5. 콘텐츠 변경 시 캠페인을 중지하고 새 소스 버전을 저장·OS별 검수한 뒤 저장·재게시합니다. SDK와 호스트 연결을 배포한 앱은 콘텐츠 변경 때마다 앱 업데이트가 필요하지 않습니다.

광고가 없거나 기본 3초 준비 기한을 넘기면 메인으로 진입합니다. 최초 동의·딥링크·권한 요청 등 제외 조건은 호스트가 연결합니다. 표시 시간은 HTML이 아닌 SDK 설정입니다. [운영 가이드](https://nudgeon.io/ko/guide/#launch-ads), [개발자센터](https://developer.nudgeon.io/#launch-ads), [전체 계약](APP-LAUNCH-ADS.md)을 참조하세요.
...
**설정 → MCP**에서 서버 주소를 복사하고 자신의 연결을 관리합니다. 관리자는 조직 연결과 고객 프로필 접근 승인도 관리합니다. AI 클라이언트 연결 시 표시되는 클라이언트 ID·콜백 주소를 확인한 뒤 앱 하나와 권한을 선택하세요. 이름은 클라이언트가 입력한 값입니다. 기본 읽기 권한에도 사용자 작성 조건·메시지 본문이 포함될 수 있습니다.

**세그먼트 → 초안**에서는 AI 또는 콘솔에서 만든 초안을 편집·미리보기하고 검토한 버전을 운영 세그먼트로 승격합니다. 저장되지 않은 수정이나 오래된 버전으로는 승격할 수 없습니다. 승격된 초안은 잠기며 필요하면 복제합니다. 저니 초안은 기존 편집기에서 검토하며, 다른 편집자가 먼저 저장한 경우 현재 입력을 유지한 채 충돌을 알려 줍니다.

역할별 범위, 고객 정보 동의, 분석 지표의 의미는 [MCP 가이드](MCP.md)를 참고하세요. MCP는 기본 비활성화이며 운영자가 설정한 경우에 연결할 수 있습니다.
