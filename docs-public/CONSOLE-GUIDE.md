...
콘솔은 브라우저의 `Accept-Language`로 첫 언어를 정하고(영어 브라우저면 영어), 대시보드 우상단의 언어 선택으로 바꿀 수 있습니다(쿠키 `NEXT_LOCALE`, URL은 그대로). **2026-09-10 현재 영문화 범위**: 로그인·가입·대시보드·온보딩 위저드(4단계). 세그먼트·저니 편집기·리포트·설정·채널 화면은 아직 한국어입니다 — 카탈로그(`apps/console/src/messages/{ko,en}.json`)에 키를 추가하고 `useTranslations`로 치환하면 됩니다. ko 카탈로그가 권위이고, en에 빠진 키는 테스트가 막습니다.


### 가입 · 로그인

셀프호스팅 기본값은 `MODE=single_tenant`입니다. 이 모드에서는 가입 화면이 없고, `./nudgeon up`이 보여준 설치 코드 링크(`/setup#token=…`)에서 **설치 소유권을 확인한 뒤 첫 Owner·워크스페이스·앱**을 만듭니다. 그 화면에 SDK Key와 Server Key가 **한 번만** 표시되니 복사해 두세요 — 분실하면 온보딩 1단계의 "SDK Key 회전"으로 새 키를 받습니다. 설치가 잠기면 로그인 화면에는 가입 링크가 없습니다. `MODE=multi_tenant`(SaaS)는 가입마다 새 테넌트를 만듭니다.
