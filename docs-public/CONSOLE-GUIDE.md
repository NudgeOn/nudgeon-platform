...
운영 노출·참여·닫힘과 실패 기록은 테스트 이력과 분리됩니다. 서버 기능 플래그와 신규 DB migration, 앱의 인앱 SDK 모듈 연결이 필요합니다. iOS SPM과 Android Maven의 코어·인앱 SDK 0.2.2가 공개되어 있으며, RN/Flutter의 인앱 API는 후속 작업입니다. [작업실 안내](IN-APP-WORKBENCH.md)와 [캠페인 게시·SDK 연결](IN-APP-CAMPAIGNS.md)을 참조하세요.
