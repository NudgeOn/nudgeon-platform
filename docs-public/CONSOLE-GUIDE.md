...
저니의 진입·전환·조건 분기·이벤트 대기와 세그먼트의 이벤트 조건에서도 **기본 이벤트 선택**을 사용할 수 있습니다. 선택 후 이름을 직접 입력해 커스텀 이벤트로 바꿀 수 있습니다. [이벤트 이름과 연동 규약](API.md#기본-이벤트)을 참고하세요. 아래 데이터 캡처는 이벤트 탭 추가 전 화면입니다.
