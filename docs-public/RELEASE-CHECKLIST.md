...
**2026-09-16 재점검:** [남은 작업과 우선순위](BETA-REMAINING-2026-09-16.md). iOS·Android 0.1.2 공개 배포를 재확인했다. RN·Flutter 공개 패키지는 아직 404이며, SDK 0.1.2 실단말 재검증과 godspell 스테이징 리허설은 완료 증거가 더 필요하다.
...
| # | 항목 | 필요한 것 | 완료 조건 |
|---|---|---|---|
| B-1 | SDK 0.1.2 배포 — 워크플로 준비 완료, 자격증명만 남음 | Android **완료** (`io.nudgeon:nudgeon-sdk:0.1.2` Maven Central, Quickstart CI 통과). RN: 시크릿 `NPM_TOKEN` → *Publish to npm* 재실행. Flutter: 첫 게시는 사람이 `flutter pub publish`(pub.dev 정책) → Automated publishing 켜기. iOS 완료 | 4 SDK 공개 좌표로 신규 앱 설치·시작·실수신 |
