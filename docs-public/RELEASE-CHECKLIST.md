...
- **공개됨:** iOS SPM 및 Android Maven Central core/in-app **0.2.4**. [iOS 릴리스](https://github.com/NudgeOn/nudgeon-ios-sdk/releases/tag/0.2.4) · [Android 릴리스](https://github.com/NudgeOn/nudgeon-android-sdk/releases/tag/0.2.4). 공개 패키지를 소비하는 Godspell 빌드와 시작 광고 기본 4초 자동 종료를 확인했습니다. iPhone·Android 에뮬레이터는 화면 확인, Fold3는 런타임 이벤트 확인이며 물리 화면 캡처는 미확인입니다.
...
아래 날짜별 기록은 당시 검증 결과입니다. 현재 버전 판정은 위 요약과 §2를 사용합니다.

**2026-09-18 인앱 검증 추가:** [0.2.2 후보 실기기·콘솔 결과](IN-APP-DEVICE-QA-2026-09-17.md). KST 숨김·정상 중단·폴드 레이아웃은 확인했지만, B-2의 푸시 3상태·중복 억제와 B-7의 실제 거래성 푸시 리허설을 대신하지 않습니다. 아래 베타 게이트는 해당 증거가 확보될 때 닫습니다.

**2026-09-18 재점검:** [남은 작업과 증거](BETA-REMAINING-2026-09-18.md). 네이티브 0.2.2 공개 배포·Godspell 반영 완료. RN/Flutter 0.1.3 준비·CI·머지 완료이며 로그인 후 게시한다. 관리형 저장소·목표 부하·24시간·외부 온보딩은 미완료, 실단말 푸시는 이번 작업에서 제외했다.
...
| # | 항목 | 필요한 것 | 완료 조건 |
|---|---|---|---|
| B-1 | SDK 공개 배포 | iOS SPM·Android core/in-app **0.2.4 완료**. RN/Flutter **0.1.3 준비·CI·머지 완료**, npm/pub.dev/CocoaPods 로그인·게시 대기 | 공개 registry-only 신규 앱 설치·계약 검증 |
