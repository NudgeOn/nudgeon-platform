...
목표: **실제 단말에서 Onda 푸시 1건을 받고 탭해서, 도달/오픈 리포트에 같은 `message_id`로 잡히는 것**을 확인한다.
이것이 베타 선언의 하드 게이트(G0)이며, 성공하면 `docs/dev` Go/No-Go 원장의 IT-3·IT-8·M-1을 Go로 올린다.

전제: 샘플앱(iOS `onda-ios-sdk/Examples/OndaDemo`, Android `onda-android-sdk/sample-app`)이 빌드된다. 소요 시간은 크리덴셜이 준비돼 있으면 약 1시간.
...
| Onda 실행 환경 | `docker compose … --profile full --profile app up -d` | 단말에서 접근 가능한 API 주소 필요 (아래 1단계) |

## 1. Onda를 단말이 닿는 주소로 띄우기
...
```bash
# 세션 쿠키는 로그인 응답의 onda_session. Owner/Admin만 가능.
curl -X PUT http://<host>:8080/v1/apps/<app_id>/credentials \
  -H 'Content-Type: application/json' -b 'onda_session=<cookie>' \
  -d '{"kind":"push_fcm","service_account": <서비스계정 JSON 그대로>}'

curl -X PUT http://<host>:8080/v1/apps/<app_id>/credentials \
  -H 'Content-Type: application/json' -b 'onda_session=<cookie>' \
  -d '{"kind":"push_apns","p8":"-----BEGIN PRIVATE KEY-----\n…\n-----END PRIVATE KEY-----","key_id":"ABC123DEFG","team_id":"TEAM123456","bundle_id":"io.onda.demo","environment":"sandbox"}'
```
...
- **iOS**: `Examples/OndaDemo`의 설정(스킴 환경 변수 또는 `Config.xcconfig`)에 `ONDA_SDK_KEY=pk_…`, `ONDA_API_HOST=http://<host>:8080`. 서명 팀·Bundle ID를 APNs 키에 등록한 값과 맞추고 **Push Notifications capability**와 NSE 타깃의 App Group을 확인한 뒤 실기기에 설치.
- **Android**: `sample-app/app/google-services.json` 배치, `local.properties`(gitignore)에 `onda.sdkKey=pk_…`, `onda.apiHost=http://<host>:8080`. `./gradlew :sample-app:installDebug`.
...
curl -X POST http://<host>:8080/v1/apps/<app_id>/test-push \
  -H 'Content-Type: application/json' -b 'onda_session=<cookie>' \
  -d '{"external_id":"<샘플앱에서 identify한 값>","title":"Onda 실기기 1호","body":"이 알림을 탭하세요"}'
...
1. 단말에 알림이 뜬다. iOS는 NSE가 `onda.message_id`를 읽어 **`$push_delivered`** 를 보낸다(백그라운드/종료 상태 모두). Android는 `onMessageReceived`에서 표시와 동시에 보고한다.
2. 알림을 **탭**한다 → 앱이 열리며 **`$push_opened`** 전송, 딥링크가 있으면 라우팅.
3. 대사: 아래 세 곳의 `message_id`가 같아야 한다.

| 위치 | 확인 방법 | 기대 |
|---|---|---|
| 발송 로그 | 콘솔 `/logs` 또는 ClickHouse `onda.message_log` | `status=sent`, `channel=push_fcm|push_apns` |
...
```sql
-- ClickHouse (docker exec -it onda-clickhouse clickhouse-client)
SELECT message_id, channel, status, sent_at FROM onda.message_log ORDER BY sent_at DESC LIMIT 5;
SELECT event_name, JSONExtractString(properties, 'message_id') AS mid, occurred_at
  FROM onda.events WHERE event_name IN ('$push_delivered', '$push_opened') ORDER BY occurred_at DESC LIMIT 5;
```
...
| 알림은 오는데 SDK가 무시 | `onda.message_id`/`message_id` 없음 | 워커 버전 확인 (`PUSH-CONTRACT.md` R-01 이후) |
