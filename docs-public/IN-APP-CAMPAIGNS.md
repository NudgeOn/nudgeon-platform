...
- 시작 전면 광고: SDK 0.2.4의 `enableAfterLaunch`와 `launch` 조건으로 연결합니다. 실제 표시부터 기본 4초(3~5초 설정) 후 자동 종료하며 네이티브 닫기·숨김 버튼은 표시하지 않습니다. 작업실 검수는 기존 테스트 흐름으로 마치고 게시된 테스트 캠페인의 앱 재실행으로 자동 종료를 확인합니다. [앱 실행 직후 광고](APP-LAUNCH-ADS.md)의 준비 시간·중복 방지 규칙을 따릅니다.
