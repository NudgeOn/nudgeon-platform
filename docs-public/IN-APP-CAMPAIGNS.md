...
2026-09-17. 웹 소스 작업실 다음 단계로 게시·기간·트리거·설치 기기별 제한과 iOS/Android 운영 클라이언트를 구현했습니다. SDK 0.2.0과 호환되며 운영 서버에는 해당 API·마이그레이션을 먼저 배포해야 합니다. 전체 기획의 세그먼트·사용자 개인화·저니 연동은 포함하지 않습니다.
...
4. 이름, 불변 소스 버전, 대상 OS, 표시 조건, 시작/종료 시각과 빈도를 설정하고 **초안 저장**합니다.
...
- 날짜 입력은 브라우저 현재 시간대, 서버 저장과 일일 제한·**오늘 그만 보기**는 UTC 기준입니다. 오늘 그만 보기는 기기·캠페인 기준이며 다음 UTC 자정까지 유지됩니다.
...
두 기능은 기본 비활성입니다. Compose 기본·Safe Boot 환경에도 변수를 전달하도록 추가했습니다. `./nudgeon up` 설치 위자드의 자동 콘텐츠 호스트 설정은 아직 포함하지 않습니다.
...
결과 화면의 노출·참여·닫힘은 운영 실행별 집계이며 테스트 로그와 분리됩니다. ClickHouse 리포트·사용자별 전환 귀속·세그먼트·저니·RN/Flutter 어댑터는 후속 작업입니다. 개발 브랜치 빌드와 시뮬레이터 검증은 Godspell 실제 배포 및 다양한 실기기 검증을 대체하지 않습니다.

## SDK 0.2.0 / P0 운영 준비

- iOS SPM의 `NudgeOnInApp` product, Android Maven Central의 `io.nudgeon:nudgeon-inapp:0.2.0`을 앱에 추가합니다. 코어 SDK도 0.2.0을 사용합니다.
