...
워커가 실제로 읽는 곳은 환경변수 `NUDGEON_CONNECTOR_MANIFESTS`이고, 기본값은 `/etc/nudgeon/connectors`다
...
이 디렉터리는 로컬에서 `NUDGEON_CONNECTOR_MANIFESTS`를 여기로 가리켜 커넥터 로딩을
...
- `runtime.type = in_process_go`인데 Go 구현이 등록돼 있지 않으면 기동이 실패한다.
  ...
  목 벤더는 `GO_TAGS=nudgeon_mock` 빌드에서만 임포트된다(`cmd/worker/connectors_mock.go`).
