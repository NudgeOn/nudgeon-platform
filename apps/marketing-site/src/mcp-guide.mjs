export const mcpGuide = {
  ko: {
    ...
    intro: 'MCP를 활성화한 설치에서는 외부 AI에 지표 정의와 집계를 제공해 성과·퍼널·리텐션을 분석하고 세그먼트와 저니 초안을 준비할 수 있습니다. NudgeOn SDK 키를 AI에 전달하지 않고 본인 계정으로 연결합니다.',
    ...
    steps: [
      ...
      ['고객 상세 조회는 별도로 승인받으세요', '일반 멤버는 먼저 읽기 연결을 만들고 관리자에게 해당 연결의 고객 상세 접근 승인을 요청합니다. 승인 후 같은 클라이언트·앱으로 다시 연결해 고객 조회에 동의하세요. 승인만으로 기존 토큰에 권한이 추가되지는 않습니다. 여러 앱을 분석하려면 앱마다 연결이 필요합니다.'],
      ...
      ['권한 오류나 저장 충돌을 해결하세요', '초안 작성이 보이지 않으면 Editor 이상 역할과 연결 시 초안 편집 동의를 확인하세요. 저장 충돌이 나면 최신 버전을 다시 읽고 변경사항을 비교합니다. 연결을 끝낼 때는 설정 → AI 연결(MCP)에서 해제하세요. 관리자에게 고객 접근 승인을 철회해 달라고 요청할 수도 있습니다.'],
    ],
  ...
  en: {
    ...
    intro: 'Installations with MCP enabled can share metric definitions and aggregates with an external AI to analyze performance, funnels and retention, then prepare segment and journey drafts. Connect with your own account instead of giving the AI a NudgeOn SDK key.',
    ...
    steps: [
      ...
      ['Request customer access separately', 'Members first create a read connection and ask an administrator to approve customer access for that connection. Reconnect with the same client registration and app, then consent to customer access. Approval alone does not expand existing tokens. Use a separate connection for each app.'],
      ...
      ['Resolve permission errors or save conflicts', 'If draft tools are missing, check that your role is Editor or higher and that you consented to draft editing. If a save conflicts, read the latest revision and compare changes. Disconnect from Settings → AI connections (MCP) when finished, or ask an administrator to revoke customer access.'],
    ],
