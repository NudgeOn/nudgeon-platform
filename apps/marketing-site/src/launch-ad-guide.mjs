...
export const launchAdGuide = {
  ko: {
    ...
    steps: [
      ['개발 담당자와 시작 화면을 연결하세요', 'iOS·Android SDK 0.2.6의 인앱 모듈과 앱 시작 흐름 연결이 필요합니다. 서버의 인앱 기능도 켜져 있어야 합니다. 먼저 테스트 앱과 기기를 준비하세요. 최초 동의·딥링크·권한 요청처럼 다른 화면이 우선이면 시작 광고를 건너뛰도록 개발 담당자와 정합니다.'],
  ...
  en: {
    ...
    steps: [
      ['Connect the startup flow with your developer', 'Install the iOS or Android in-app SDK 0.2.6 and integrate the host startup flow. Enable the server in-app features and prepare a controlled test app and device. Agree on when to skip startup ads, including initial consent, deep links and permission prompts.'],
