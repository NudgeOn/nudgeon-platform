 ...
 * - 마스터키: ONDA_MASTER_KEY(base64 32B) 또는 KMS_MASTER_KEY_FILE. SaaS는 v1.5에 KMS 교체.
 */

const NONCE_LEN = 12;

export function loadMasterKey(env: NodeJS.ProcessEnv = process.env): Buffer {
  const inline = env.ONDA_MASTER_KEY;
  if (inline) {
    const key = Buffer.from(inline, "base64");
    if (key.length !== 32) throw new Error("ONDA_MASTER_KEY는 base64 32바이트여야 합니다");
    return key;
  }
  ...
  throw new Error("마스터키 미설정 — ONDA_MASTER_KEY 또는 KMS_MASTER_KEY_FILE 필요");
}
