...
export function EventNameInput({ value, onValueChange, ...props }: Props) {
  const t = useTranslations("eventCatalog");
  const known = STANDARD_EVENTS.some((event) => event.name === value);
  return <span className="flex min-w-0 flex-col gap-2">
    <select aria-label={t("choose")} disabled={props.disabled} value={known ? value : ""}
      ...
      {STANDARD_EVENTS.map((event) => <option key={event.name} value={event.name}>
        {t(`events.${event.name}.label`)} · {event.name}
      </option>)}
    </select>
