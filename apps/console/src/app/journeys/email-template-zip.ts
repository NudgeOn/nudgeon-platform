...
export interface ZipMessage { key: string; params?: Record<string, string | number> }
...
function archiveError(key: string, params?: Record<string, string | number>): EmailTemplateZipError {
  return new EmailTemplateZipError(key, params);
}

/** 같은 경고는 한 번만 — key+params 직렬화로 중복 제거한다. */
class WarningSet {
  private readonly items = new Map<string, ZipMessage>();
  add(key: string, params?: Record<string, string | number>): void {
    const message: ZipMessage = params ? { key, params } : { key };
    this.items.set(JSON.stringify(message), message);
  }
  list(): ZipMessage[] { return [...this.items.values()]; }
}
    ...
    try {
      unzip(data, {
        filter: (entry) => {
          ...
          if (!path) {
            rejected ??= archiveError("zip.unsafePath");
            return false;
          }
          ...
          if (fileCount > MAX_FILE_COUNT) {
            rejected ??= archiveError("zip.tooManyFiles", { max: MAX_FILE_COUNT });
            return false;
          }
          if (entry.originalSize > MAX_ENTRY_BYTES) {
            rejected ??= archiveError("zip.entryTooLarge");
            return false;
          }
          if (expandedBytes > MAX_EXPANDED_BYTES) {
            rejected ??= archiveError("zip.expandedTooLarge");
            return false;
          }
          if (entry.originalSize > 1024 * 1024 && entry.size > 0 && entry.originalSize / entry.size > 100) {
            rejected ??= archiveError("zip.suspiciousRatio");
            return false;
          }
          if (/\.zip$/i.test(path)) {
            rejected ??= archiveError("zip.nestedZip");
            return false;
          }
          return true;
        },
        ...
        if (error) {
          reject(archiveError("zip.cannotOpenCorrupt"));
          return;
        }
        ...
        for (const [rawPath, bytes] of Object.entries(entries)) {
          ...
          if (files.has(path) || caseInsensitivePaths.has(foldedPath)) {
            reject(archiveError("zip.duplicatePath"));
            return;
          }
      ...
      reject(archiveError("zip.cannotOpenFormat"));
    }
  });
}
...
function assertSafeMarkup(html: string): void {
  if (hasUnsafeEmailMarkup(html)) {
    throw archiveError("zip.unsafeHtml");
  }
}

function rewriteCssAssets(css: string, fromPath: string, files: Map<string, Uint8Array>, warnings: WarningSet): string {
  const withoutImports = css.replace(/@import\s+(?:url\()?[^;]+;?/gi, () => {
    warnings.add("zip.warning.removedStyleImport");
    return "";
  });
  return withoutImports.replace(/url\(\s*(["']?)(.*?)\1\s*\)/gi, (full, _quote: string, rawReference: string) => {
    ...
    if (!path) {
      warnings.add("zip.warning.blockedExternalAssets");
      return "url(\"\")";
    }
    const bytes = files.get(path);
    const uri = bytes && dataUri(path, bytes);
    if (!uri) {
      warnings.add("zip.warning.missingReference", { reference });
      return "url(\"\")";
    }
    return `url(\"${uri}\")`;
  });
...
function buildStandaloneHtml(rawHtml: string, entryPath: string, files: Map<string, Uint8Array>) {
  ...
  if (typeof DOMParser === "undefined") throw archiveError("zip.noDomParser");

  const warnings = new WarningSet();
  ...
  document.querySelectorAll<HTMLLinkElement>("link[rel~='stylesheet' i]").forEach((link) => {
    ...
    if (!path || !bytes) {
      warnings.add("zip.warning.removedExternalStylesheet");
      link.remove();
      return;
    }
    ...
    try {
      ...
      throw archiveError("zip.cssNotUtf8", { path });
    }
  ...
  document.querySelectorAll<HTMLElement>("img[src], source[src], [background]").forEach((element) => {
    ...
    if (!uri) {
      warnings.add("zip.warning.blockedExternalImages");
  ...
  return { html: `<!doctype html>\n${document.documentElement.outerHTML}`, warnings: warnings.list() };
}
...
export async function importEmailTemplateZipBytes(data: Uint8Array, archiveName: string, archiveSize = data.byteLength): Promise<ImportedEmailTemplate> {
  if (archiveSize > MAX_ZIP_BYTES || data.byteLength > MAX_ZIP_BYTES) {
    throw archiveError("zip.zipTooLarge");
  }
  const files = await unzipArchive(data);
  if (files.size === 0) throw archiveError("zip.noUsableFiles");
...
export async function importEmailTemplateZip(file: File): Promise<ImportedEmailTemplate> {
  if (!/\.zip$/i.test(file.name)) throw archiveError("zip.selectZip");
  if (file.size > MAX_ZIP_BYTES) throw archiveError("zip.zipTooLarge");
