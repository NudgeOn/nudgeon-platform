...
export function JourneyAppGate({ children }: { children: (appId: string) => ReactNode }) {
  return <Suspense><AppGate>{children}</AppGate></Suspense>;
}

function AppGate({ children }: { children: (appId: string) => ReactNode }) {
  ...
  return <AppIdProvider value={appId}>{children(appId)}</AppIdProvider>;
}
