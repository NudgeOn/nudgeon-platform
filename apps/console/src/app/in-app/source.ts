...
export function launchExampleFiles(): InAppFile[] {
  return [
    { path: "index.html", base64: encode(`<!doctype html><html lang="ko"><head><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><link rel="stylesheet" href="styles.css"></head><body><main><header>NUDGEON <span>광고</span></header><section aria-labelledby="ad-title"><h1 id="ad-title">오늘의 새 소식</h1><p>새 콘텐츠가 도착했어요.</p><div class="art" aria-hidden="true">✦</div></section><footer>잠시 후 메인 화면으로 이동합니다</footer></main></body></html>`) },
    // Keep the same content available without a timer in the host app's announcements.
    // Review at 200% text size and with VoiceOver/TalkBack before publishing.
    { path: "styles.css", base64: encode(`*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#142e25;color:#f1f7dc;font-family:system-ui,sans-serif}main{min-height:100vh;min-height:100dvh;display:flex;flex-direction:column;gap:1.5rem;padding:max(28px,env(safe-area-inset-top)) max(30px,env(safe-area-inset-right)) max(28px,env(safe-area-inset-bottom)) max(30px,env(safe-area-inset-left));overflow-wrap:anywhere}header{display:flex;flex-wrap:wrap;justify-content:space-between;gap:1rem;font-size:1rem;font-weight:700}section{flex:1;display:flex;flex-direction:column;justify-content:center}h1{font-size:clamp(2rem,8vw + 1rem,4rem);line-height:1.25;margin:0 0 1.5rem}p{font-size:1.25rem;line-height:1.7;margin:0}.art{align-self:flex-end;font-size:8rem;line-height:1;color:#d5ea94;margin-top:1.5rem}footer{font-size:1rem;line-height:1.7}@media(max-height:540px){.art{display:none}main{gap:1rem}h1{margin-bottom:1rem}}`) },
  ];
}
