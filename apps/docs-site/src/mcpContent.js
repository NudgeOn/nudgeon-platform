...
# Remote MCP endpoint shown in Settings
...
const discovery = `curl --fail https://api.example.com/.well-known/oauth-protected-resource/mcp
curl --fail https://api.example.com/.well-known/oauth-authorization-server`;

const messages = `// query_message_metrics tool arguments
// Results include observed outcomes from first send through as_of.
{
  "interval": "week",
  "group_by": "channel",
  "compare": true
...
const segment = `// create_segment_draft tool arguments
// Generate a UUID for each new create; reuse it for retries of the same input.
{
  "name": "KR customers",
  "request_id": "3efad286-cf6a-4cda-9a7e-f7b8fcb117c0",
  "definition": {
    "version": 1,
    "operator": "AND",
    "groups": [{
      "operator": "AND",
      "conditions": [{"type": "attribute", "key": "country", "op": "eq", "value": "KR"}]
    }]
  }
}`;

// Pin new contracts to the implemented version so public links also work before PR merge.
const contractBase = "https://github.com/NudgeOn/nudgeon-platform/blob/5d2e12aa9554e831cb4e2c212e000ec17ecae598";

const links = [
  { label: "MCP · API / OAuth / tool contracts", href: `${contractBase}/docs-public/MCP.md` },
  { label: "Connection & draft OpenAPI", href: `${contractBase}/packages/openapi/mcp.openapi.json` },
  { label: "Analytics OpenAPI", href: `${contractBase}/packages/openapi/mcp-analytics.openapi.json` },
  { label: "OAuth & HTTP transport OpenAPI", href: `${contractBase}/packages/openapi/mcp-oauth.openapi.json` },
  { label: "Deployment", href: `${contractBase}/docs-public/DEPLOY.md` },
];
...
export const mcpContent = {
  ko: {
    ...
    steps: [
      { title: "설치에서 MCP를 활성화합니다", body: "MCP가 포함된 API·콘솔 버전과 추가 DB 마이그레이션 0013·0014를 배포한 뒤 MCP_ENABLED를 활성화하세요. 기본값은 false입니다. MCP_PUBLIC_URL에는 외부에서 접근할 API origin을, MCP_CONSOLE_URL에는 콘솔 origin을 입력합니다. 운영에서는 HTTPS를 사용하고 loopback HTTP는 로컬 개발에만 사용하세요. URL에 /mcp나 /api 경로를 붙이지 마세요. 프록시는 /mcp·/oauth/*·discovery를 API로, /mcp/authorize를 콘솔로 전달하며 원래 Host와 포트를 유지해야 합니다. 별도 모델 API 키나 내장 AI는 필요하지 않습니다." },
      ...
      { title: "비교 기준과 품질을 함께 읽습니다", body: "기본 기간은 앱 시간대의 종료된 최근 30일입니다. 날짜 지정 시 start_date 포함, end_date 제외이며 최대 90일, 퍼널·리텐션 유입 기간은 최대 30일입니다. 행동 시각은 기본 client_ts이며 server_ts 선택이 가능합니다. 메시지는 최초 발송 시각으로 코호트를 정하고 발송 이후 조회 시점(as_of)까지의 관측 결과를 합산하며 time_basis 인수를 받지 않습니다. 퍼널은 첫 시작 후 기본 7일 안의 순서를 따르며 같은 시각의 이벤트 순서는 모호함으로 표시합니다. 비율은 1이 100%이고 분모가 0이면 null, 미성숙 리텐션은 not_matured입니다. 기준 시각·분모·품질 경고를 AI 답변에도 포함하게 하세요." },
      ...
      { title: "고객 상세 접근은 승인 후 다시 동의합니다", body: "일반 멤버는 먼저 mcp:read 연결을 만듭니다. Admin·Owner가 그 연결의 고객 접근을 승인하면, 같은 등록 client_id와 앱으로 다시 인증해 mcp:customers:read에 동의하세요. 기존 액세스 토큰의 권한은 승인만으로 늘어나지 않습니다. 클라이언트 등록을 새로 만들었다면 별도 연결로 취급하므로 승인 대상을 다시 확인합니다." },
      { title: "재시도와 연결 해제를 처리합니다", body: "생성 작업마다 UUID request_id를 만들고 같은 입력의 재시도에는 재사용합니다. 수정 전에는 최신 revision을 읽어 expected_revision으로 보내세요. REVISION_CONFLICT는 다시 읽고 비교한 뒤 수정하고, QUERY_LIMIT는 동시 작업을 줄이거나 잠시 뒤 재시도합니다. INSUFFICIENT_SCOPE 또는 도구가 보이지 않으면 역할과 동의 범위를 확인합니다. CONNECTION_EXPIRED는 클라이언트의 토큰 갱신 또는 재인증으로 처리합니다. 연결을 폐기하면 재인증이 필요합니다. 환경변수 MCP_ENABLED=false는 서버를 비활성화하며 기존 연결의 영구 해제와는 구분됩니다." },
    ],
    examples: [
      { label: "운영자 설정 · 기본 비활성", code: setup },
      { label: "OAuth discovery 확인", code: discovery },
      ...
      { label: "채널별 메시지 성과 비교", code: messages },
      { label: "운영 평가에 사용되지 않는 세그먼트 초안 생성", code: segment },
    ],
  ...
  en: {
    ...
    endpoint: "OAuth + Streamable HTTP · /mcp",
    steps: [
      { title: "Enable MCP for your installation", body: "Deploy an API and console version that includes MCP, with additive migrations 0013 and 0014, then enable MCP_ENABLED. It defaults to false. Set MCP_PUBLIC_URL to the externally reachable API origin and MCP_CONSOLE_URL to the console origin. Use HTTPS in production; loopback HTTP is for local development. Do not append /mcp or /api to these origins. The proxy must route /mcp, /oauth/* and discovery to the API, but /mcp/authorize to the console, preserving the original Host and port. No separate model API key or built-in AI model is required." },
      ...
      { title: "Consent again after customer-access approval", body: "Members first create an mcp:read connection. After an Admin or Owner approves customer access for it, authorize again using the same registered client_id and app, and consent to mcp:customers:read. Approval alone does not add scope to an existing access token. A new client registration creates a separate connection, so check which connection is approved." },
      { title: "Handle retries and disconnects", body: "Generate a UUID request_id for each create and reuse it when retrying the same input. Read the latest revision before editing and pass it as expected_revision. For REVISION_CONFLICT, reread and compare edits; for QUERY_LIMIT, reduce concurrent work or retry later. If tools are missing or return INSUFFICIENT_SCOPE, check role and consent. CONNECTION_EXPIRED requires token refresh or reauthentication through the client. A revoked connection requires reauthorization. MCP_ENABLED=false disables the server; it does not permanently revoke existing connections." },
    ],
    examples: [
      { label: "Operator configuration · disabled by default", code: setup },
      { label: "Check OAuth discovery", code: discovery },
      ...
      { label: "Message performance by channel", code: messages },
      { label: "Create a non-operational segment draft", code: segment },
    ],
