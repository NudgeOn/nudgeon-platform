...
export const launchAdContent = {
  ko: {
    ...
    intro: 'APP-AD는 해당 앱의 신규·기존 사용자 전체가 대상입니다. 로그인·세그먼트·테스트 기기 연결 없이 선택한 OS의 SDK 연결 기기에서 동의·기간·빈도 조건에 따라 표시합니다. OS 런치 화면 → 앱의 시작 화면 덮개 → 불투명 전면 광고 → 메인 화면. SDK 0.2.6는 실제 표시부터 기본 4초 뒤 광고를 자동 종료합니다. 표시 시간은 3~5초로 설정하며 시작 광고에는 네이티브 닫기·오늘 하루 안 보기 버튼이 없습니다.',
    endpoint: 'enableAfterLaunch(timeoutSeconds: 3, displaySeconds: 4) · trigger: launch',
    steps: [
      { title: '서버와 네이티브 SDK를 준비합니다', body: 'launch 트리거를 지원하는 서버·콘솔과 migration 0010~0012를 먼저 반영합니다. IN_APP_ENABLED와 IN_APP_CAMPAIGNS_ENABLED를 켜고 영속 자산 볼륨·별도 HTTPS 콘텐츠 호스트를 설정합니다. SPM NudgeOnInApp과 Maven nudgeon-inapp/core 0.2.6를 사용합니다. RN·Flutter의 인앱 연결은 아직 제공하지 않습니다.' },
    ...
    examples: [
      { label: 'iOS · SPM 0.2.6 + NudgeOnInApp product', code: '.package(url: "https://github.com/NudgeOn/nudgeon-ios-sdk.git", exact: "0.2.6")\n// Add .product(name: "NudgeOnInApp", package: "nudgeon-ios-sdk") to your target.' },
      { label: 'iOS · 전체 실행 소스 펼치기 / 복사', code: swift, complete: true },
      { label: 'Android · Maven Central 의존성', code: 'implementation("io.nudgeon:nudgeon-sdk:0.2.6")\nimplementation("io.nudgeon:nudgeon-inapp:0.2.6")\nimplementation("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.8.1")' },
    ...
    source: 'APP-LAUNCH-ADS · IN-APP-CAMPAIGNS · iOS/Android SDK 0.2.6', links: links.ko,
  },
  en: {
    ...
    intro: 'APP-AD targets all new and existing users of this app. No login, segment membership or test pairing is required; SDK integration, consent, schedule and frequency still apply on selected platforms. OS launch screen → host startup cover → opaque full-screen ad → main screen. SDK 0.2.6 automatically dismisses the ad 4 seconds after presentation by default. Configure 3–5 seconds; startup ads have no native close or hide-today buttons.',
    endpoint: 'enableAfterLaunch(timeoutSeconds: 3, displaySeconds: 4) · trigger: launch',
    steps: [
      { title: 'Prepare the server and native SDKs', body: 'Deploy the launch-capable server/console and migrations 0010–0012 first. Enable IN_APP_ENABLED and IN_APP_CAMPAIGNS_ENABLED, persistent asset storage and a separate HTTPS content host. Use SPM NudgeOnInApp and Maven in-app/core 0.2.6. React Native and Flutter in-app integration is not yet available.' },
    ...
    examples: [
      ...
      { label: 'Android · Maven Central dependencies', code: 'implementation("io.nudgeon:nudgeon-sdk:0.2.6")\nimplementation("io.nudgeon:nudgeon-inapp:0.2.6")\nimplementation("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.8.1")' },
    ...
    source: 'APP-LAUNCH-ADS · IN-APP-CAMPAIGNS · iOS/Android SDK 0.2.6', links: links.en,
  },
};
