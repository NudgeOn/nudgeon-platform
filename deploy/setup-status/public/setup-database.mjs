...
export function initDatabaseSetup() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    request ??= { request_id: crypto.randomUUID(), password: password.value };
    submit.disabled = true; password.disabled = true; $("#generate-database-password").disabled = true;
    try {
      ...
      if (!response.ok) {
        ...
        result.textContent = json.error === "invalid_token" ? "설치 코드가 올바르지 않습니다. 터미널에서 코드를 확인해 주세요."
    ...
    finally { if (!submitted) { password.disabled = false; $("#generate-database-password").disabled = false; update(); } }
  });
